...
<head>
    ...
    <title>Health Quiz Results</title>
    <style>
        body {
...
<body>
    <%
        ...
        try {
            ...
            // Get user answers and correct answers for the health quiz
            PreparedStatement userAnswersStmt = conn.prepareStatement(
                "SELECT hq.question_id, hq.question, hq.correct_option, hq.opt1, hq.opt2, hq.opt3, hq.opt4, COALESCE(ua.selected_option, 0) AS selected_option " +
                "FROM health_questions hq " +
                "LEFT JOIN healthuser_answers ua ON hq.question_id = ua.question_id AND ua.user_answer_id = (SELECT MAX(user_answer_id) FROM healthuser_answers WHERE question_id = hq.question_id)"
            );
